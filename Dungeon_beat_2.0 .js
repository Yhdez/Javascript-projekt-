...
var Times_moved_counter = null
var Times_attacked_counter = null
...
// Här defineras constructors som används vid uppdatering och skapelsen av objekt

function Karaktär(type,swidth,sheight,x,y,width,height,sx,sy,health) { //Denna används vid Spelkaraktären och current_monster (Det nuvarande monstret spelaren möter)
    this.type = type
    this.width = width;
    this.height = height;
    this.x = x;
    ...
    this.update = function (){
        if (Facing === "Downward"){ //Sätter den nuvarande positionen spelarens sprite ska vara i
          this.sy = 0;
          Player_attack.sy = 340 //Används för att slash sprite'n ska vara framför spelaren när den ritas
        ...
        if (invinciblity_frame_mode === "Active"){ //Ifall spelaren blir träffad av en projektil eller monster, säts Invincibility_frame_mode till true, detta ser till att sprite'n ritas som transparent medans denna variabel är true. Säts vid Is_player_hit()
    ...
    this.Monster_update = function(){ // Current_monster har en individuell .update för att inte påverka spelkaraktärens sprite
...
function Assets(type,x,y,width,height,swidth,sheight,sx,sy){ //Används vid projektiler och för Player_attack (Slash sprite'n)
    this.type = type
    this.width = width
    this.height = height
    this.x = x
    ...
    this.sx = sx
    this.sy = sy
    this.update = function (){; //Används för projektiler då de inte använder sy och sx
      ctx.drawImage(this.type,this.x,this.y,this.width,this.height)
      Current_room.update()
      Check_health()
    }
    this.update_with_s = function(){ //Används av Player_attack
      ctx.drawImage(this.type,this.sx,this.sy,this.swidth,this.sheight,this.x,this.y,this.width,this.height)
    }
}


function Room(type,wall_type,Top_border,Bottom_border,Left_border,right_border){ //Används för de olika rummen och väggarna i rummen
   this.type = type
...
function Check_health(){ //Uppdaterar bilden av sprite'n som visar spelarens liv
...
function End_scene(){ //Gör i princip samma sak som player_death(), fast anger end_scene_active till true som ser till att spelaren kan starta om spelet vid tryck av "Enter"
...
document.onkeydown = function(e){
    switch (e.key){
        ...
        case "ArrowUp":
            ctx.clearRect(Spelkaraktär.x,Spelkaraktär.y,Spelkaraktär.width,Spelkaraktär.height)
            Facing = "Upward"
            if (Spelkaraktär.y >= Current_room.Top_border){ //Ser till att spelaren inte kan gå framåt trots att den står vid en väg (border)
    ...
    switch (e.key){ //Spelaren försöker att öppna, tillgänglig när spelaren rör en dörr eller en kista
        case "e":
         ...
         if (player_is_touching_chest === true && chest_status === "Unopened"){ //Spelaren öppnar en kista, som inte är redan öppnad. Kistan är av typen current_monster eftersom att den har en sannolikhet av vara ett monster (Mimic) och eftersom att då krävs inte uppdattering av ett ytterligare objekt
            let Chest_result = Math.ceil((Math.random()*2))
            ctx.clearRect(Current_monster.x,Current_monster.y,Current_monster.width,Current_monster.height)
            if (Chest_result === 1){ //Kistan öppnas och spelaren helar med +3 health
                chest_status = "Opened"
    ...
    //Vid varje tryck av en knapp, kollar den vart spelaren är relativt med andra objekt
    is_player_at_door() 
    is_player_at_chest()
...
function Player_animation(Character_action){
    ...
    if (Character_action === "Walking"){ //Animation när spelaren går 
        Times_moved_counter++
        var Current_frame = 0
        var Frame_to_end_with = 3
        var Interval = setInterval(animate,200)
    }
    if (Character_action === "Attack"){ //Animation när spelaren slår (Trycker på z)
        Times_attacked_counter++
        var Current_frame = 3
        var Frame_to_end_with = 8
        Spelkaraktär.sx = 2310
    ...
    function animate(){ //Hanteras i intervaler då denna funktion återupprepas efter en viss mängd tid.
        Current_frame++
        ctx.clearRect(Spelkaraktär.x,Spelkaraktär.y,Spelkaraktär.width,Spelkaraktär.height)
        ...
        Current_room.update()
        Check_health()
        if (Current_frame === Frame_to_end_with){ //Efter att intervalet har pågått så många gånger som den var definerad att återupprepa, avslutas animationen
            clearInterval(Interval)
            ctx.clearRect(Spelkaraktär.x,Spelkaraktär.y,Spelkaraktär.width,Spelkaraktär.height)
            Spelkaraktär.sx = 0
            Spelkaraktär.update()
            Current_monster.Monster_update()
            if (player_attack_active === true){Player_attack.update_with_s()} //Då slash sprite'n endast ska ritas om den håller på, alltså  att player_attack_active === true
            Monster_projectile_asset.update()
            Monster_projectile_asset_2.update()
            Monster_projectile_asset_3.update()
    ...
    //Här definerar objekt som behöver vara definerade vid spelets början, dessa uttnytjar constructorerna längst upp i filen, den uppdatterar även objekt så att de ska vara ritande vid spelets början
    ...
    var Current_room = new Room("Bilder/Rooms/Main_Dungeon_background.png",document.getElementById("Main_Dungeon_walls"),Game_canvas.height*0.24,Game_canvas.height*0.78,-35,Game_canvas.width*0.86)
